...
*/
	...
	RALINK_REG(RALINK_SYSCTL_BASE+0x60)&=~(1<<9);
	RALINK_REG(RALINK_SYSCTL_BASE+0x60)|= (1<<8);
	...
	{		
		...
		for (i=0; i<10; ++i)
		{
			...
			{	
				BootType = '5';
				printf("Load firmware Automatically\n");
				goto update;
			}
	...
	while (timer1 > 0) {
		...
		for (i=0; i<100; ++i) {
			if ((my_tmp = tstc()) != 0) {	/* we got a key press	*/
				...
				BootType = getc();
				if ((BootType < '0' || BootType > '5') && (BootType != '7') && (BootType != '8') && (BootType != '9')&& (BootType != '5'))
					BootType = '3';
				printf("\n\rYou choosed %c\n\n", BootType);
				break;
			}
